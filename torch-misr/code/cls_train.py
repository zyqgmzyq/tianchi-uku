...
cls_net = models.resnet18(pretrained=True)
cls_net.fc = nn.Linear(2048, 2)
print(cls_net)
...
for epoch in range(config.start_epoch, config.nEpochs + 1):
    ...
    for iteration, batch in enumerate(training_data_loader):
        ...
        data_uv -= 127.5
        data_uv /= 127.5
        data_uv_2x = data_uv.view(3 * data_uv.shape[0], -1).\
            repeat(1, 2 ** 2).view(3 * 2 ** 2, -1).\
            view(data_uv.shape[0], 3 * 2 ** 2, data_uv.shape[2], data_uv.shape[3])
    ...
    upsample = common.Upsampler(common.default_conv, scale=2, n_feats=10, act=False)
    if epoch % 2 == 0:
