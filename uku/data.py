...
def get_training_set(data_dir, hr, lr, upscale_factor, patch_size, data_augmentation):
...
def get_eval_set(data_dir, hr, lr, upscale_factor):
    hr_dir = join(data_dir, hr)
    lr_dir = join(data_dir, lr)
...
def get_test_set(data_dir, lr, upscale_factor):
    lr_dir = join(data_dir, lr)
    return DatasetFromFolderTest(lr_dir, upscale_factor, transform=transform())
