...
for epoch in range(args.repoch, args.epochs):
    ...
    for iteration, batch in enumerate(train_loader):
        lr = batch[0]
        label = batch[1]
        label_down4 = F.interpolate(label, scale_factor=0.25, mode="bicubic", align_corners=True)
        label1 = F.interpolate(label_down4, scale_factor=4, mode="bicubic", align_corners=True)
        label_down2 = F.interpolate(label, scale_factor=0.5, mode="bicubic", align_corners=True)
        label2 = F.interpolate(label_down2, scale_factor=2, mode="bicubic", align_corners=True)
        # print(label2.size())
        sr1, sr2, sr3 = net(lr.to(device))
        loss1 = criterion(sr1, label1.to(device))
        loss2 = criterion(sr2, label2.to(device))
        loss3 = criterion(sr3, label.to(device))
    ...
    with torch.no_grad():
        ...
        for tlrs, thrs in val_loader:
            tsrs1, tsrs2, tsrs3 = net(tlrs.to(device))
            tsrs = tsrs3.permute(0, 2, 3, 1)
